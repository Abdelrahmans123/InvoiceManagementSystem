                        ...
                        <div class="row mg-t-20">
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>رقم الفاتورة</span>
                                    <span>{{ $invoices->invoiceNumber }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاصدار</span>
                                    <span>{{ $invoices->invoiceDate }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاستحقاق</span>
                                    <span>{{ $invoices->dueDate }}</span></p>
                                <p class="invoice-info-row"><span>القسم</span>
                                    <span>{{ $invoices->department->departmentName }}</span></p>
                            </div>
                        </div>
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                <tr>
                                    ...
                                    <td class="tx-center">{{ number_format($invoices->amountCollection, 2) }}</td>
                                    <td class="tx-right">{{ number_format($invoices->amountCommission, 2) }}</td>
                                    @php
                                        $total = $invoices->amountCollection + $invoices->amountCommission ;
                                    @endphp
                                    <td class="tx-right">
                                ...
                                <tr>
                                    <td class="tx-right">نسبة الضريبة ({{ $invoices->rateVat }})</td>
